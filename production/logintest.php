    ...
    if ($row)
        ...
        if (pg_num_rows($logon) > 0)
            ...
            if ($row_arrays['club_id'] == 'BITD')
                ...
                if ($row_arrays['position'] == 'topmanagement')
                    {
                    session_start();
                    $_SESSION['userid'] = $matricno;
                    session_write_close();
...
if (isset($_POST['submit']))
	...
	if ($username != '' && $password != '')
		...
		if ($row)
			...
			echo '</script>'; / echo "<script>window.location.href='customer_booking.php?username=$username';</script>"; /
			}
		  else
			{
			include ('db_connect.php');
			...
			$table = mysqli_fetch_assoc($bot); / $lies = mysqli_fetch_row($table); / /
			if ($lies1) / /
				{/
				if ($table['staff_role'] == 'admin')
					...
					/*$_SESSION['log_id'] = true;
					$_SESSION['user']="admin";*/
					$_SESSION['username'] = $username; / session_write_close(); / mysqli_close($conn);
					echo "<script>window.location.href='admin/index.php?username=$username';</script>";
					}
				  else
				if ($table['staff_role'] == 'staff')
					{
					session_start();
					$_SESSION['log_id'] = true;
					$_SESSION['user'] = "staff";
					...
					session_write_close();
					mysqli_close($conn);
					echo "<script>window.location.href='staff/index.php?username=$username';</script>"; / echo "<script>window.location.href='admin/index.php';</script>"; /
					}
